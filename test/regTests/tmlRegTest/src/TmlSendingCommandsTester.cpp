    ...
    if(createCore(0) && createCore(1))
    {
      if(createListener(1, 0, sAddress1))
      {
        if(startListener(1, 0))
        {
            ...
            //m_iErr = tml_Send_SyncMessage(NULL, tCommandEmpty, S_IO_PROFILE , S_TD_127_0_0_1, tmlrt_itoa(iPort), 3000);
			...
			//m_iErr = tml_Send_SyncMessage(getCore(0), NULL, S_IO_PROFILE, S_TD_127_0_0_1, tmlrt_itoa(iPort), 3000);
			...
			checkForExpectedReturnCode(TML_ERR_MISSING_OBJ, tmlrtT("tml_Send_SyncMessage(address)"));

			// Check for NULL as port
			m_iErr = tml_Send_SyncMessage(getCore(0), tCommand, S_IO_PROFILE, S_TD_127_0_0_1, NULL, 3000);
			checkForExpectedReturnCode(TML_ERR_MISSING_OBJ, tmlrtT("tml_Send_SyncMessage(NULLasPort)"));
...
void TmlSendingCommandsTester::listenerCallBack(TML_COMMAND_HANDLE tmlhandle, TML_POINTER pCBData)
{
	...
	//check.m_iErr = tml_Send_AsyncStatusReply(tmlhandle, NULL, tmlrtT("Test"));
    ...
    if(createCore(0) && createCore(1))
    {
      if(createListener(1, 0, sAddress1))
      {
        if(startListener(1, 0))
        {
			...
			// Check for NULL as core handle
			//m_iErr = tml_Send_AsyncMessage(NULL, tCommand, S_IO_PROFILE, S_TD_127_0_0_1, tmlrt_itoa(iPort), 3000);
			...
			checkForExpectedReturnCode(TML_ERR_SENDER_PROFILE_NOT_SUPPORTED, tmlrtT("tml_Send_AsyncMessage(emptyCommandHandle)"));

			// Check for NULL as command handle
			//m_iErr = tml_Send_AsyncMessage(getCore(0), NULL, S_IO_PROFILE, S_TD_127_0_0_1, tmlrt_itoa(iPort), 3000);
			//checkForExpectedReturnCode(TML_ERR_SENDER_PROFILE_NOT_SUPPORTED, tmlrtT("tml_Send_AsyncMessage(NULLasCommandHandle)"));
			...
			checkForExpectedReturnCode(TML_ERR_SENDER_INVALID_PARAMS, tmlrtT("tml_Send_AsyncMessage(NULLasHostAddress)"));
			...
			checkForExpectedReturnCode(TML_ERR_SENDER_INVALID_PARAMS, tmlrtT("tml_Send_AsyncMessage(NULLasPort)"));
